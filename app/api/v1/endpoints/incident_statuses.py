...
from app.schemas.incident_status import IncidentStatusCreate, IncidentStatusRead, IncidentStatusUpdate
...
async def create_incident_status(data: IncidentStatusCreate, session: DbSession, _admin: SuperAdminUser):
...
async def list_incident_statuses(session: DbSession, _current_user: CurrentUser, skip: int = 0, limit: int = 100):
    svc = IncidentStatusService(session)
    return await svc.get_all(skip=skip, limit=limit)


@router.patch("/{status_id}", response_model=IncidentStatusRead)
async def update_incident_status(status_id: int, data: IncidentStatusUpdate, session: DbSession, _admin: SuperAdminUser):
    svc = IncidentStatusService(session)
